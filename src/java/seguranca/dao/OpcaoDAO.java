...
public class OpcaoDAO extends BaseDAO<TbOpcao> {
     public List<TbOpcao> gerarOpcoes(int menu , String username, String password) {
        List<TbOpcao> lista;
        Query qy = hib.createQuery("SELECT opcao  FROM TbOpcao opcao,TaPrivilegio privilegio, TbPerfil perfil, TbUsuario usuario "
                ...
                + "AND usuario.usrUsuario LIKE ? AND usuario.pwdUsuario LIKE ?  and opcao.tbMenu LIKE ? ORDER BY opcao.ordOpcao");
        qy.setString(0, username);
        qy.setString(1, password);
        qy.setInteger(2, menu);
        lista = qy.list();
        return lista;
    }
